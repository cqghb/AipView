<template>
	<div>
		<!-- 产品规格选项管理 -->
		<base-table :uri="uri"
		            ref="skuSpecificationOptionTable"
		            ...
		            :formSize="formSize"
					:multiSelect="false"
					searchFormRef="searchskuSpecificationOptionForm"
...
<script>
	...
	export default {
		...
		data() {
			return {
				uri: SystemConstant.consSkuSpecificationOptionManage.FIND_PAGE,
				searchData:{
					id: "",// 产品主键
					code: "",// 产品代码
					name: "",// 节点名称
				...
				searchHandle:[
					{
						...
						handle:()=>{
							let _this = this;
							_this.loading = true;
							_this.$refs.skuSpecificationOptionTable.queryList();// 调子组件的方法
						}
					},
					{
						label:"重置",
						...
						handle:()=>{
							let _this = this;
							_this.$refs.skuSpecificationOptionTable.$refs.searchskuSpecificationOptionForm.$refs.defaultMyForm.resetFields();
						}
					}
				],
				formSize: "",
				...
				tableColumnList:[
					...
					{ prop: "skuId", label: "SKU产品主键", width: 180 },
					{ prop: "skuCode", label: "SKU产品代码", width: 180 },
					{ prop: "skuName", label: "SKU产品名称", width: 180 },
					{ prop: "id", label: "SPU规格主键", width: 180 },
					{ prop: "specName", label: "SPU规格名称", width: 180 },
					{ prop: "specOptiId", label: "SPU规格选项ID", width: 180 },
					{ prop: "specOptiValue", label: "规格选项值", width: 180 },
		...
		methods: {
			...
			update(){// 修改
				let _this = this;
				_this.$refs.skuSpecificationOptionTable.commonCheck();
				if(_this.$refs.skuSpecificationOptionTable.selectedData){
					let params = _this.$refs.skuSpecificationOptionTable.selectedDataArr[0];
					let param = {
					        id: params.id
					    };
			...
			/* delete(){// 删除 删除之后只是不让再配置到商品规格中去
				...
				_this.$refs.skuSpecificationOptionTable.commonCheck();
				if(_this.$refs.skuSpecificationOptionTable.selectedData){
					let selectedItem = _this.$refs.skuSpecificationOptionTable.selectedDataArr[0];
					...
					CommInterface.sendPost(SystemConstant.consSpecificationManage.UPDATE_DEL_TAG, {id: id}, function(num){
						if(num>0){
							...
							_this.$refs.skuSpecificationOptionTable.loading = true;
							_this.$refs.skuSpecificationOptionTable.queryList();
						} else {
